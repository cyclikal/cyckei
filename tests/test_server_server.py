...
def basic_cellrunner():
    ...
    test_runner = protocols.CellRunner(
        {"testing": "plugins", "second": "test"}, **test_meta)
...
def test_info_all_channels(basic_cellrunner):
    ...
    test_info = server.info_all_channels(
        [basic_cellrunner], [basic_cellrunner.source])
    ...
    assert channel_a["current"] == 0.01
    assert channel_a["voltage"] > 2.800
    assert channel_a["voltage"] < 2.802
...
def test_info_channel(basic_cellrunner):
    ...
    basic_cellrunner.load_protocol(
        "from cyckei.server import protocols\nprotocols.CurrentStep(0.01)")
    basic_cellrunner.run()
    test_dict = server.info_channel('a', [basic_cellrunner], [
                                    basic_cellrunner.source])
    assert test_dict["channel"] == 'a'
    assert test_dict["status"] == 'started'
    assert test_dict["state"] == 'charge_constant_current'
    assert test_dict["current"] == 0.01
    assert test_dict["voltage"] > 2.800
    assert test_dict["voltage"] < 2.802
...
def test_start(basic_cellrunner):
    ...
    assert server.start('a', test_meta, test_protocol, [], [
                        basic_cellrunner.source], []) == "Succeeded in starting channel a."
    assert server.start('a', test_meta, test_protocol, [basic_cellrunner], [
                        basic_cellrunner.source], []) == "Channel a already in use."
...
def test_resume(basic_cellrunner):
    ...
    assert server.resume('a', [basic_cellrunner]
                         ) == "Succeeded in resuming channel a"
...
def test_get_runner_by_channel(basic_cellrunner):
    ...
    assert server.get_runner_by_channel(
        "a", [basic_cellrunner]) == basic_cellrunner
    assert server.get_runner_by_channel(
        "a", [basic_cellrunner], 0) == basic_cellrunner
